...
export function Footer() {
  return (
    <footer className="bg-[#06182D] py-12 text-white">
      <div className="container grid gap-10 lg:grid-cols-[1.2fr_.8fr_1fr]">
        <div>
          ...
          <p className="mt-4 text-sm leading-7 text-white/62">Email: <a className="text-[#C9A24A]" href={`mailto:${site.email}`}>{site.email}</a><br />Phone / WhatsApp: <a className="text-[#C9A24A]" href={`https://wa.me/${site.whatsappNumber.replace(/\D/g, '')}`}>{site.phone}</a><br />Click to call: <a className="text-[#C9A24A]" href={`tel:${site.phone.replace(/\s/g, '')}`}>{site.phone}</a><br />Hours: {site.businessHours}</p>
        </div>
        <div>
          <p className="footer-title">Navigate</p>
          <div className="mt-4 grid gap-2 text-sm text-white/65">
            {navItems.map((item) => <Link key={item.href} href={item.href} className="hover:text-[#C9A24A]">{item.label}</Link>)}
          </div>
        </div>
        <div>
