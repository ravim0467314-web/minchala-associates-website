...
export function ContactForm() {
  ...
  return (
    <form className="grid gap-4" onSubmit={submit} noValidate>
      ...
      <label className="flex gap-3 rounded-2xl border border-white/10 bg-white/[0.06] p-4 text-sm leading-6 text-white/65"><input className="mt-1 h-4 w-4 accent-[#C9A24A]" type="checkbox" checked={form.consent} onChange={(event: ChangeEvent<HTMLInputElement>) => update('consent', event.target.checked)} /> <span>I consent to Minchala & Associates contacting me using the details shared for evaluating this consultation / appointment request.</span></label>
