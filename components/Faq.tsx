  ...
  return (
    <section className="section-pad bg-white"><JsonLd data={faqJsonLd} />
      <div className="container grid gap-10 lg:grid-cols-[0.75fr_1.25fr]">
        <div>
          <p className="eyebrow text-[#b99655]">FAQs</p>
          <h2 className="section-title">Responsible answers before engagement.</h2>
        </div>
        <div className="space-y-4">
          {faqs.map((item) => (
            <details key={item.question} className="group rounded-2xl border border-slate-200 bg-[#f7f3ea] p-6 open:bg-white open:shadow-sm">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-6 font-bold text-[#061724]">
                {item.question}<span className="text-[#b99655] transition group-open:rotate-45">＋</span>
              </summary>
