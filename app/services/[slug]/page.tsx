      ...
      <section className="section-pad">
        <div className="container grid gap-10 lg:grid-cols-[.82fr_1.18fr] lg:items-start">
          <aside className="luxury-card overflow-hidden rounded-[2rem] p-4 lg:sticky lg:top-28">
            ...
            <div className="p-5">
              <p className="text-sm font-bold uppercase tracking-[0.16em] text-[#C9A24A]">Serving Bapatla, Guntur & Hyderabad</p>
          ...
          <div>
            <div className="grid gap-4 sm:grid-cols-2">
              {service.details.map((detail) => <article key={detail} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><span className="text-[#C9A24A]">◆</span><h2 className="display mt-3 text-3xl">{detail}</h2></article>)}
            </div>
            <article className="mt-8 rounded-[2rem] bg-[#0B2341] p-8 text-white">
              <p className="eyebrow text-[#C9A24A]">Professional outcome</p>
            ...
            <section className="mt-10">
              <p className="eyebrow text-[#C9A24A]">Service FAQs</p>
              <div className="mt-5 grid gap-4">
                {service.faqs.map((faq) => <details key={faq.question} className="rounded-3xl border border-slate-200 bg-white p-6"><summary className="cursor-pointer font-bold text-[#0B2341]">{faq.question}</summary><p className="mt-4 leading-7 text-slate-600">{faq.answer}</p></details>)}
              </div>
            </section>
            <section className="mt-10 rounded-[2rem] border border-slate-200 bg-white p-7">
              <p className="eyebrow text-[#C9A24A]">Related services</p>
              <div className="mt-5 flex flex-wrap gap-3">{relatedServices.map((item) => <Link key={item.slug} href={`/services/${item.slug}`} className="rounded-full border border-slate-200 px-4 py-2 text-sm font-bold text-[#0B2341] transition hover:border-[#C9A24A]">{item.title}</Link>)}</div>
            </section>
          </div>
        </div>
      </section>
      <section className="section-pad bg-[#0B2341] text-white"><div className="container grid gap-8 lg:grid-cols-2"><div><p className="eyebrow text-[#C9A24A]">Request a consultation / appointment</p><h2 className="section-title">Start with a professional scope review.</h2><p className="mt-5 text-white/65">Use firm contact channels only. An enquiry does not create a professional engagement.</p></div><ContactForm /></div></section>
    </main>
  );
}
