...
export default async function LocationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main id="main">
        ...
        <div className="container grid gap-10 lg:grid-cols-[1fr_.9fr] lg:items-start">
          <div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {location.serviceFocus.map((item) => <article key={item} className="rounded-3xl border border-slate-200 bg-white p-6"><span className="text-[#C9A24A]">◆</span><h2 className="display mt-3 text-3xl">{item}</h2></article>)}
            </div>
          </div>
          <aside className="rounded-[2rem] bg-[#0B2341] p-8 text-white lg:sticky lg:top-28">
            <p className="eyebrow text-[#C9A24A]">Contact and directions</p>
            <p className="mt-5 leading-8 text-white/70">Email: <a className="text-[#C9A24A]" href={`mailto:${site.email}`}>{site.email}</a><br />Phone / WhatsApp: <a className="text-[#C9A24A]" href={`tel:${site.phone.replace(/\s/g, '')}`}>{site.phone}</a><br />Hours: {site.businessHours}</p>
      ...
      <section className="section-pad bg-white"><div className="container"><p className="eyebrow text-[#C9A24A]">Practice areas</p><h2 className="section-title max-w-4xl">Integrated service pages for {location.city} and nearby areas.</h2><div className="mt-8 grid gap-4 md:grid-cols-3">{services.map((service) => <Link href={`/services/${service.slug}`} key={service.slug} className="luxury-card rounded-3xl p-6"><h3 className="display text-3xl">{service.title}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{service.summary}</p></Link>)}</div></div></section>
      <section className="section-pad bg-[#0B2341] text-white"><div className="container grid gap-8 lg:grid-cols-2"><div><p className="eyebrow text-[#C9A24A]">Appointment request</p><h2 className="section-title">Discuss a {location.city} service requirement.</h2><p className="mt-5 text-white/65">Professional engagement begins only after scope, eligibility and documentation review.</p></div><ContactForm /></div></section>
    </main>
  );
}
