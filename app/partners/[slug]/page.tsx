...
export default async function PartnerProfilePage({ params }: PartnerParams) {
  ...
  return <main id="main"><JsonLd data={personJsonLd} /><Breadcrumbs items={[{ label: 'Partners', href: '/partners' }, { label: partner.name }]} /><section className="section-pad"><div className="container grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div className="luxury-card overflow-hidden rounded-3xl"><AssetImage {...partner.image} width={900} height={700} className="h-80 w-full object-cover" /><div className="p-8"><h1 className="display text-5xl">{partner.name}</h1><p className="mt-2 text-sm font-bold uppercase tracking-[0.16em] text-[#C9A24A]">{partner.credentials} · {partner.role}</p><p className="mt-6 font-semibold">{partner.focus}</p><p className="mt-4 leading-8 text-slate-600">{partner.bio}</p></div></div><div><p className="eyebrow text-[#C9A24A]">Qualifications & journey</p><div className="mt-6 grid gap-4 sm:grid-cols-2">{partner.qualifications.map((item) => <article key={item} className="rounded-3xl border border-slate-200 bg-white p-6 font-semibold text-[#0B2341]">{item}</article>)}</div><div className="mt-8 space-y-4">{partnerTimelines.map((item) => <article key={item.year} className="rounded-3xl border border-slate-200 bg-white p-6"><strong className="text-[#C9A24A]">{item.year}</strong><p className="mt-2 text-slate-600">{item.text}</p></article>)}</div><p className="mt-6 rounded-2xl bg-[#0B2341] p-5 text-sm leading-7 text-white/70">Published information is limited to approved public professional details. Specific signing authority, engagement team and scope are confirmed only through written engagement communication.</p></div></div></section></main>;
}
