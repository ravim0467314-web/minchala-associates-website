...
export default async function ArticlePage({ params }: ArticleParams) {
  ...
  return <main id="main"><JsonLd data={articleJsonLd} /><Breadcrumbs items={[{ label: 'Knowledge Hub', href: '/knowledge-hub' }, { label: article.title }]} /><article className="section-pad"><div className="container max-w-3xl"><p className="eyebrow text-[#C9A24A]">{article.category} · {article.readTime}</p><h1 className="section-title">{article.title}</h1><p className="mt-6 text-xl leading-9 text-slate-600">{article.summary}</p><div className="mt-10 rounded-3xl border border-slate-200 bg-white p-8 leading-8 text-slate-700"><p>This article is part of the Minchala & Associates knowledge system. The repository includes matching editorial source files under <code>content/articles</code> for content governance workflows.</p><p className="mt-4">The information is general in nature and should not be treated as professional advice for specific facts.</p></div></div></article></main>;
}
